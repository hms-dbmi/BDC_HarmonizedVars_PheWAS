...
dict_harmonized_variables = read_csv("env_variables/df_harmonized_variables.csv", col_types = "cccccc") %>% as.data.table()
multiindex_variablesDict = read_csv("env_variables/multiIndex_variablesDict.csv", col_types = "cccccccdlcdddccli") %>% as.data.table()
...
setnames(significant_associations, "value", "signifiance")
...
wide_significant_table <- significant_association_table %>% 
  unique(by = c("dependent_var_id", "independent_var_id", "indicator", "pvalue")) %>%
  dcast(formula = dependent_var_id + independent_var_id + pvalue ~ indicator, value.var = "value") %>%
  unique() %>%
...
categorical_significant =  wide_significant_table[harmonized_var_type %in% c("binary", "categorical"), ] %>%
  ...
  rename("Explanatory Variable" = "independent_simplified_name", 
        ...
        "Study" = "study") %>%
  select(all_of(c("Explanatory Variable", "Explained Variable (harmonized)", "OR", "CI_OR", "pvalue", "Sample Size", "Explanatory Variable (complete name)", "Study"))) %>%
  arrange(pvalue, desc(OR))
...
continuous_significant = wide_significant_table[harmonized_var_type %in% c("continuous"), ] %>% 
  ...
  rename("Explanatory Variable" = "independent_simplified_name", 
        ...
        "Sample Size" = "nonNA_crosscount", 
        "Study" = "study") %>%
  select(all_of(c("Explanatory Variable", "Explained Variable (harmonized)", "Variable Effect", "CI Variable Effect", "pvalue", "Sample Size", "Explanatory Variable (complete name)", "Study"))) %>%
  arrange(pvalue, desc(`Variable Effect`))
               

categorical_significant %>% write_csv(file.path(path_results, "association_statistics", "categorical_significant.csv"))
continuous_significant %>% write_csv(file.path(path_results, "association_statistics", "continuous_significant.csv"))
# 
